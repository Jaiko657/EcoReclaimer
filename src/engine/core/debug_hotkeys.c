...
void sys_debug_binds(const input_t* in)
{
    ...
    if (s_inspect_mode && input_pressed(in, BTN_MOUSE_L)) {
        ...
        if (best >= 0) {
            ...
            LOGC(LOGCAT_ECS, LOG_LVL_INFO,
                 ...
                 h.idx, h.gen, (uint64_t)mask, cmp_pos[best].x, cmp_pos[best].y, world.x, world.y);
    ...
    if (input_pressed(in, BTN_DEBUG_TRACE_START)) {
    ...
    if (input_pressed(in, BTN_DEBUG_TRACE_STOP)) {
        if (!prof_trace_is_active()) {
