...
static bool tileset_for_gid(const world_map_t* map, uint32_t gid, const tiled_tileset_t** out_ts, size_t* out_index)
{
    if (out_ts) *out_ts = NULL;
    if (out_index) *out_index = 0;
    if (!map || map->tileset_count == 0 || !out_ts || !out_index) return false;
                             ...
                             bool* out_flip_v)
{
    ...
    if (!tileset_for_gid(map, gid, &ts, &ts_idx)) return false;
                    ...
                    painter_queue_ctx_t* painter_ctx)
{
    ...
    while (layer_i < map->layer_count || obj_i < map->object_count) {
        ...
        while (layer_i < map->layer_count && map->layers[layer_i].z_order == z) {
            draw_tile_layer(map, tr, &map->layers[layer_i], (int)layer_i,
                startX, startY, endX, endY, now_ms, painter_ctx);
            layer_i++;
        }
