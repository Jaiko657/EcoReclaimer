...
/*
Doors are more complex than other components because they track which tiles they control.
They disable tile animations and set a base tile so the door can control visuals itself.
On destroy we just release tiles and re-enable anims
*/
...
static void door_init_tiles(cmp_door_t* d, const door_tile_xy_t* tile_xy, int tile_count)
{
    ...
    for (int i = 0; i < tile_count; ++i) {
        // Fill per-tile info from the current map layers.
        door_tile_info_t* t = &d->tiles[i];
        t->tx = tile_xy[i].x;
        t->ty = tile_xy[i].y;
        t->layer_idx = -1;
        ...
        uint32_t raw_gid = 0;
        // Pick the topmost non-empty layer at this tile. (assums doors are at top layer)
        ...
        int chosen_ts = -1;
        int local_id = 0;
        // Resolve GID into tileset/local tile id.
    ...
    // Pick a primary anim duration from the first animated tile.
    d->primary_anim_total_ms = 0;
    for (int i = 0; i < tile_count; ++i) {
        door_tile_info_t* t = &d->tiles[i];
    ...
    for (int i = 0; i < tile_count; ++i) {
        ...
        if ((size_t)t->tileset_idx >= map->tileset_count) continue;
        const tiled_tileset_t* ts = &map->tilesets[(size_t)t->tileset_idx];
        // Freeze anim and set the base tile for manual control.
...
void cmp_add_door(ecs_entity_t e, float prox_radius, int tile_count, const door_tile_xy_t* tile_xy)
{
    ...
    // reset incase already has door
    door_release_tiles(d);
    *d = (cmp_door_t){ .prox_radius = prox_radius };
    if (tile_xy && tile_count > 0) {
        //setup tiles for door control
