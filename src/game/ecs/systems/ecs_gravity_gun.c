...
static bool grav_gun_get_mouse_world(const input_t* in, gfx_vec2* out)
{
...
static void sprite_clear_component(int idx)
{
    ...
    cmp_spr[idx] = (cmp_sprite_t){ .tex = (tex_handle_t){ .idx = 0, .gen = 0 } };
...
static void tool_drop(int gun_idx, int player_idx)
{
    ...
    cmp_add_position((ecs_entity_t){ .idx = (uint32_t)gun_idx, .gen = ecs_gen[gun_idx] },
...
static int find_grab_candidate(int player_idx, gfx_vec2 mouse_world)
{
...
static void grav_gun_set_player_filter(int idx, bool ignore_player)
{
    ...
    if (ignore_player) {
        ...
        const unsigned int player_bit = phys_tag_bit("player");
        const unsigned int tardas_bit = phys_tag_bit("tardas");
        mask &= ~player_bit;
...
static void begin_hold(int idx, ecs_entity_t holder, gfx_vec2 mouse_world)
{
...
static void sys_grav_gun_input_impl(const input_t* in)
{
    ...
    if (held_idx >= 0) {
        if (!input_down(in, ACT_MOUSE_L)) {
            release_hold(held_idx);
        }
        return;
    }

    if (!input_pressed(in, ACT_MOUSE_L)) return;

    gfx_vec2 mouse_world = gfx_vec2_make(0.0f, 0.0f);
...
{
    if (!in || !input_pressed(in, ACT_INTERACT)) return;
...
static void update_held(int idx, cmp_liftable_t* g, float dt, const input_t* in)
{
    ...
    if (!in || !input_down(in, ACT_MOUSE_L)) {
        release_hold(idx);
        return;
    }
...
static void sys_grav_gun_fx_impl(void)
{
    for (int i = 0; i < ECS_MAX_ENTITIES; ++i) {
        ...
        gfx_color color = (gfx_color){ .r = 0.470588f, .g = 0.784314f, .b = 1.0f, .a = 1.0f  };
        int thickness = 1;
        ...
        if (tool_idx >= 0) {
            ...
            if (tool->max_charge > 0.0f) {
                ...
                if (ratio < 0.3f) {
                    ...
                    if (t < 0.0f) t = 0.0f;
                    if (t > 1.0f) t = 1.0f;
                    gfx_color low = (gfx_color){ .r = 1.0f, .g = 0.2f, .b = 0.2f, .a = 1.0f  };
        ...
        gfx_vec2 start = gfx_vec2_make(cmp_pos[holder_idx].x, cmp_pos[holder_idx].y);
        gfx_vec2 end = gfx_vec2_make(cmp_pos[i].x, cmp_pos[i].y);
    ...
    for (int i = 0; i < ECS_MAX_ENTITIES; ++i) {
        ...
        cmp_spr[i].fx.highlighted = true;
        cmp_spr[i].fx.highlight_color = (gfx_color){ .r = 0.2f, .g = 1.0f, .b = 0.2f, .a = 1.0f  };
    }
