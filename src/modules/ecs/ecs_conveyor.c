...
static bool conveyor_rider_eligible(int idx)
{
    if ((ecs_mask[idx] & CMP_PHYS_BODY) == 0) return false;
    if ((ecs_mask[idx] & CMP_LIFTABLE) && cmp_liftable[idx].state == GRAV_GUN_STATE_HELD) {
        return false;
    }
    return true;
}
...
static void sys_conveyor_update_impl(void)
{
    ...
    while (ecs_prox_stay_next(&it, &v)) {
        ...
        if (!conveyor_rider_eligible(rider_idx)) continue;
