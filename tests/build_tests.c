...
int main(int argc, char **argv)
{
    ...
    if (coverage) {
        const char *cover_cmd =
            ...
            "  lcov --rc geninfo_unexecuted_blocks=1 --capture --directory build/tests/obj --output-file build/tests/coverage/lcov.info >/dev/null; "
            "  lcov --rc geninfo_unexecuted_blocks=1 --remove build/tests/coverage/lcov.info '*/third_party/*' '*/tests/*' --output-file build/tests/coverage/lcov.info.cleaned >/dev/null; "
